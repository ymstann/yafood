...
(function ($) {
  ...
  var OptionManager = (function () {
    ...
    var DEFAULT_OPTIONS = {
      ...
      numberOfDecimals: null,  // 金額の四捨五入のとき、小数点以下を0桁とする（整数）現在未使用
      cartItems: null,
  ...
  var MathHelper = (function () { // このメソッドは、入力された文字列が１．数値かどうか判断、２．四捨五入の２つの機能があるが、現状は、四捨五入は整数のみに改造している
    var objToReturn = {};
    var getRoundedNumber = function (number) {  //　数値以外のときエラーとする
      if (isNaN(number)) {
        throw new Error('正しい数値を入力してください');
      }
      number = number * 1;
      // var options = OptionManager.getOptions();
      //return number.fixt(options.numberOfDecimals);
      return Math.round(number);  // 元のプログラムは、＄だったので、固定小数点だったが、日本の円では、整数だけなので修正
    };
  ...
  var loadMyCartEvent = function (targetSelector) {
    ...
    var drawTable = function () {
      ...
      $.each(products, function () {
        ...
        $cartTable.append(
          ...
          '<input type="number" min="1" webkit-appearance="none" style="width: 40px; margin: 0;text-align: right;" class="' + classProductQuantity + '" value="' + this.quantity + '"/></br>'+
      ...
      $cartTable.append(products.length ?
        ...
        '<td></td>' +
        '<td></td>' +
        '<td class="text-right" style="width: 80px;"><strong id="' + idGrandTotal + '"></strong></td>' + // 金額欄の桁数の変動で、表の幅が変化するのを防ぐためセルのwidth指定をした
        '<td></td>' +
        '</tr>' :
    ...
    */
    ...
    $(document).on("input", "." + classProductQuantity, function () {   //　キーボードから数量を直接入力されたときの処理
    ...
    $(document).on("click", ".btn-updown", function () {
      ...
      $cartBadge.text(ProductManager.getTotalQuantity()); //　金額の総合計をする
      showGrandTotal();
      showDiscountPrice();
    });


    $(document).on('keypress', "." + classProductQuantity, function (evt) { // 数字の0～9の範囲のキーボード入力であることチェック、数字以外は無視する
